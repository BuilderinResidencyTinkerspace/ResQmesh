...
#define BATTERY_MONITOR_ENABLED 0           // 0 = bypass ADC (fixed healthy 3.85V), 1 = active ADC sensing
#define PIN_BATTERY_ADC         8           // GPIO8 / ADC1_CH7
#define BATTERY_ADC_UNIT        1           // ADC Unit 1
#define BATTERY_ADC_CHANNEL     7           // ADC Channel 7
// NOTE: The solder pads on the back of the XIAO ESP32-S3 (BAT+ / BAT-) provide power
// to the board's 3.3V regulator and charging IC, but are NOT internally connected to any ADC pin.
// - Set BATTERY_MONITOR_ENABLED to 0 if powering via the back pads without an external resistor divider.
// - Set BATTERY_MONITOR_ENABLED to 1 if you wire an external 100k/100k divider from BAT+ to an ADC pin.
...
#define IMU_MAX_CALIB_MOTION    0.15f       // Max allowable g deviation during calibration
...
// Attitude limits
#define MAX_LEGAL_TILT_DEG      60.0f       // Beyond this tilt angle, emergency disarm fires
...
// =============================================================================
// OUTER LOOP: Angle Controller (Angle Error [deg] -> Desired Angular Rate [deg/s])
...
// INNER LOOP: Angular Rate Controller (Rate Error [deg/s] -> Mixer Torque Output)
...
// Rate loop derivative low-pass filter cutoff (reduces motor buzz/heat from gyro noise)
...
// If motor saturation occurs, throttle is lowered to preserve roll/pitch attitude authority.
